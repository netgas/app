...
trait BaseAuthConfig  extends AuthConfig {
  ...
  type Id = Long
  type User = Account
  type Authority = Seq[String]

  val idTag: ClassTag[Id] = classTag[Id]
  val sessionTimeoutInSeconds = 3600

  def resolveUser(id: Id)(implicit ctx: ExecutionContext) = Future.successful(models.DAO.AccountDAO.findById(id))
  ...
  override def authorizationFailed(request: RequestHeader, user: User, authority: Option[Authority])(implicit ctx: ExecutionContext) = {
    ...
    Future.successful(Forbidden("Nu aveti permisiunea necesara pentru a accesa aceasta pagina!"))
  }

  //versiunea noua care tine cont de multiple roluri
